window.addEventListener('load', function () {
    ...
    const tabla = $('#areaTabla').DataTable({
        scrollY: '500px',
        scrollCollapse: true,
        responsive: true,
        ajax: {
            url: 'php/area_ajax.php',
            type: 'POST',
            data: function (d) {
                d.accion = 'leer_todos';
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    const estado = parseInt(row.area_estado);
                    let botones = '';

                    if (estado === 1) {
                        botones += `
                            <div class="acciones">
                                <div class="icon-action" data-modal-target="new_area" title="Actualizar">
                                    <img src="img/icons/actualizar.png" alt="Actualizar">
                                </div>
                                <div class="icon-action btn_ver_info" data-modal-target="info_area" data-id="${row.area_id}" title="Info">
                                    <img src="img/icons/info.png" alt="Info">
                                </div>
                                <div class="icon-action btn_eliminar" data-id="${row.area_id}" title="Eliminar">
                        ...
                        botones += `
                            <div class="acciones">
                                ...
                                <div class="icon-action btn_recuperar" data-id="${row.area_id}" title="Recuperar">
    ...
    // Acción: Actualizar
    $('#areaTabla tbody').on('click', '.icon-action[title="Actualizar"]', function () {
        const fila = tabla.row($(this).closest('tr')).data();
        if (fila?.area_id) abrirFormularioEdicionArea(fila.area_id);
    });

    // Acción: Ver info
    $('#areaTabla tbody').on('click', '.btn_ver_info', function () {
        ...
        .then(res => res.json())
        .then(data => {
            if (data.exito && data.area) {
                mostrarInfoArea(data.area);
            }
        });
    });

    // Eliminar
    $('#areaTabla tbody').on('click', '.btn_eliminar', function () {
        const id = $(this).data('id');
        if (!id) return;
        ...
        .then(data => {
            if (data.exito && data.area) {
                mostrarConfirmacionArea(data.area, 'eliminar');
            }
        });
    });

    // Recuperar
    $('#areaTabla tbody').on('click', '.btn_recuperar', function () {
        const id = $(this).data('id');
        if (!id) return;
        ...
        .then(data => {
            if (data.exito && data.area) {
                mostrarConfirmacionArea(data.area, 'recuperar');
            }
        });
    });
});
