window.addEventListener('load', function () {
    if (typeof $ !== 'function') {
        console.error('jQuery no está disponible');
        return;
    }

    let estadoActual = 1;
    const toggleBtn = document.getElementById('toggleEstado');

    if (toggleBtn) {
        toggleBtn.textContent = 'Deshabilitados';
        toggleBtn.classList.add('estado-rojo');

        toggleBtn.addEventListener('click', () => {
            estadoActual = estadoActual === 0 ? 1 : 0;

            if (estadoActual === 0) {
                toggleBtn.textContent = 'Habilitados';
                toggleBtn.classList.remove('estado-rojo');
                toggleBtn.classList.add('estado-verde');
            } else {
                toggleBtn.textContent = 'Deshabilitados';
                toggleBtn.classList.remove('estado-verde');
                toggleBtn.classList.add('estado-rojo');
            }
            tabla.ajax.reload(null, false);
        });
    }

    const tabla = $('#personaTabla').DataTable({
        scrollY: '500px',
        scrollCollapse: true,
        responsive: true,
        ajax: {
            url: 'php/personal_ajax.php',
            type: 'POST',
            data: function (d) {
                d.accion = 'leer_todos';
                d.estado = estadoActual;
                d.cargo_id = document.getElementById('cargo_filtro')?.value || '';
            },
            dataSrc: 'data',
            error: function (xhr, status, error) {
                console.error('Error AJAX:', error);
                console.log('Respuesta del servidor:', xhr.responseText);
            }
        },
        columns: [
            { data: 'persona_id', title: 'ID' },
            { data: 'persona_nombre', title: 'Nombre' },
            { data: 'persona_apellido', title: 'Apellido' },
            { data: 'cargo_nombre', title: 'Cargo' },
            { data: 'persona_cedula', title: 'Cédula' },
            { data: 'persona_correo', title: 'Correo' },
            { data: 'persona_telefono', title: 'Teléfono' },
            { data: 'persona_foto', title: 'Foto',
                render: function (data) {
                    const foto = data && data.trim() !== '' ? data : 'img/icons/personal.png';
                    return `
                        <div class="imagen_celda_wrapper">
                            <img src="${foto}?t=${new Date().getTime()}" class="tabla_imagen">
                        </div>
                    `;
                },
                orderable: false
            },
            { data: null, title: 'Acciones',
                render: function (row) {
                    const estado = parseInt(row.persona_estado);
                    let botones = '';
                    if (estado === 1) {
                        botones += `
                            <div class="acciones">
                                <div class="icon-action" data-modal-target="new_personal" title="Actualizar">
                                    <img src="img/icons/actualizar.png" alt="Actualizar">
                                </div>
                                <div class="icon-action btn_ver_info" data-modal-target="info_persona" data-id="${row.persona_id}" title="Info">
                                    <img src="img/icons/info.png" alt="Info">
                                </div>
                                <div class="icon-action btn_eliminar" data-id="${row.persona_id}" title="Eliminar">
                                    <img src="img/icons/eliminar.png" alt="Eliminar">
                                </div>
                            </div>
                        `;
                    } else {
                        botones += `
                            <div class="acciones">
                                <div class="icon-action btn_ver_info" data-modal-target="info_persona" data-id="${row.persona_id}" title="Info">
                                    <img src="img/icons/info.png" alt="Info">
                                </div>
                                <div class="icon-action btn_recuperar" data-id="${row.persona_id}" title="Recuperar">
                                    <img src="img/icons/recuperar.png" alt="Recuperar">
                                </div>
                            </div>
                        `;
                    }
                    return botones;
                },
                orderable: false
            }
        ],
        paging: true,
        info: true,
        dom: '<"top"Bf>rt<"bottom"lpi><"clear">',
        buttons: ['excel', 'pdf'],
        language: {
            search: "Buscar:",
            lengthMenu: "Mostrar _MENU_ registros",
            zeroRecords: "No se encontraron registros coincidentes",
            emptyTable: "No hay ningún registro",
            info: "Mostrando _START_ a _END_ de _TOTAL_ registros",
            infoEmpty: "Mostrando 0 a 0 de 0 registros",
            infoFiltered: "(filtrado de _MAX_ registros totales)",
            paginate: {
                previous: "◀",
                next: "▶"
            }
        },
        lengthMenu: [[5, 10, 15, 20, 30], [5, 10, 15, 20, 30]],
        pageLength: 15
    });

    // Eventos de acción
    $('#personaTabla tbody').on('click', '.icon-action[title="Actualizar"]', function () {
        const fila = tabla.row($(this).closest('tr')).data();
        if (fila && fila.persona_id) {
            abrirFormularioEdicionPersona(fila.persona_id);
        }
    });

    $('#personaTabla tbody').on('click', '.btn_ver_info', function () {
        const id = $(this).data('id');
        if (!id) return;

        fetch('php/personal_ajax.php', {
            method: 'POST',
            body: new URLSearchParams({ accion: 'obtener_persona', persona_id: id })
        })
        .then(res => res.json())
        .then(data => {
            if (data.exito && data.persona) {
                mostrarInfoPersona(data.persona);
            }
        });
    });

    $('#personaTabla tbody').on('click', '.btn_eliminar', function () {
        const id = $(this).data('id');
        if (!id) return;

        fetch('php/personal_ajax.php', {
            method: 'POST',
            body: new URLSearchParams({ accion: 'obtener_persona', persona_id: id })
        })
        .then(res => res.json())
        .then(data => {
            if (data.exito && data.persona) {
                mostrarConfirmacionPersona(data.persona, 'eliminar');
            }
        });
    });

    $('#personaTabla tbody').on('click', '.btn_recuperar', function () {
        const id = $(this).data('id');
        if (!id) return;

        fetch('php/personal_ajax.php', {
            method: 'POST',
            body: new URLSearchParams({ accion: 'obtener_persona', persona_id: id })
        })
        .then(res => res.json())
        .then(data => {
            if (data.exito && data.persona) {
                mostrarConfirmacionPersona(data.persona, 'recuperar');
            }
        });
    });
});
