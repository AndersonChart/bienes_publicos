...
class PDF_Recepcion_General extends FPDF {
    ...
    function Header() {
        ...
        if (file_exists($membrete)) {
            // Ancho casi completo (márgenes 10mm)
...
if (empty($recepciones)) {
    $pdf->Cell(0,10,utf8_decode('No existen recepciones registradas.'),0,1,'C');
...
$pdf->MultiCell(0,6, utf8_decode(
    "     El presente documento constituye el Reporte General de Recepciones Institucionales, el cual consolida todas las actas de incorporación de bienes patrimoniales registradas en el sistema. ".
...
foreach ($recepciones as $r) {
    ...
    $pdf->Cell(0,6, date('d/m/Y', strtotime($r['ajuste_fecha'])),0,1,'L');

    $pdf->Cell(40,6,utf8_decode('Descripción:'),0,0,'L');
    $pdf->MultiCell(0,6, utf8_decode($r['ajuste_descripcion'] ?: '—'));

    $pdf->Cell(40,6,'Estado:',0,0,'L');
    $estadoTxt = ($r['ajuste_estado'] == 1) ? 'Activa' : 'Anulada';
    $pdf->Cell(0,6,$estadoTxt,0,1,'L');
    ...
    $pdf->Cell(40,7,utf8_decode('SERIAL'),1,0,'C',true);
    ...
    $pdf->Cell(30,7,utf8_decode('CATEGORÍA'),1,1,'C',true);
    ...
    if (empty($items)) {
        $pdf->Cell(190,7,'No hay artículos registrados en esta recepción.',1,1,'C');
    } else {
        foreach ($items as $it) {

            $pdf->Cell(40,7, $it['articulo_serial'],1,0,'C');
            $pdf->Cell(60,7, utf8_decode($it['articulo_nombre']),1,0,'L');
            $pdf->Cell(30,7, utf8_decode($it['articulo_modelo'] ?: '—'),1,0,'L');
            $pdf->Cell(30,7, utf8_decode($it['marca_nombre']),1,0,'L');
            $pdf->Cell(30,7, utf8_decode($it['categoria_nombre']),1,1,'L');
        }
    }
